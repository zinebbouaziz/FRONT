...
export default function WorkspacePage() {
  ...
  // ─── Resolve project ID: URL → localStorage ─────────────────────────────
  ...
  // ─── Orchestrator HITL state (write / compound intents) ─────────────────
  ...
  // ─── Sources refresh trigger (increment after upload / search approval) ──
  ...
  // ─── Fetch chat, literature, suggestions ────────────────────────────────
  useEffect(() => {
    if (!project?.id) return;
    apiFetch(`/projects/${project.id}/chat`)
  ...
  // Load current document version when active section changes
  useEffect(() => {
  ...
  const handleSave = useCallback(() => {
    ...
    // Use orchestrator to save: write intent then auto-approve as human edit
    apiFetch(`/projects/${project.id}/run`, {
      method: 'POST',
      body: JSON.stringify({
        user_message: `Save content to section "${activeSection.title}"`,
        section_id: activeSection.id,
      }),
    })
      .then((res: any) => {
        const threadId = res.thread_id;
        return apiFetch(`/projects/${project.id}/run/${threadId}/resume`, {
          method: 'POST',
          body: JSON.stringify({
            hitl_action: 'edit',
            human_edited_text: content,
          }),
        });
      })
      .then(() => {
  ...
  }, [editor, activeSection?.id, project?.id]);
  ...
  const sendMessage = useCallback(() => {
    ...
    setChatInput('');
    setChatLoading(true);
    // Clear any previous HITL state before a new run
    ...
    apiFetch(`/projects/${project.id}/run`, {
      ...
      .then((res: any) => {
        // Always echo the agent_output as an AI message so the user sees it
        const aiMsg: Message = {
        ...
        // For write / compound intents that require HITL approval, park the thread
        ...
        // After a search intent completes, refresh the library panel
  ...
  const handleHitlApprove = useCallback(async () => {
    try {
      ...
      setHitlThreadId(null);
      setHitlAgentOutput(null);
      // Refresh versions after a write is approved
      if (activeSection?.id) fetchVersions(activeSection.id);
      // Refresh sources if the approved thread had a search intent
      if (hitlIntents.includes('search')) setSourcesRefreshKey((k) => k + 1);
    } catch (err: any) {
      setHitlStatus('error');
    }
  }, [resumeThread, project, activeSection?.id, hitlIntents, fetchVersions]);
  ...
  }, [resumeThread, project, activeSection?.id, fetchVersions]);
  ...
  const handleHitlRegenerate = useCallback(async (feedback: string) => {
    ...
    try {
      ...
      // If the backend looped and is still pending_review, update the parked output
  ...
  }, [resumeThread, project, activeSection?.id, hitlThreadId, fetchVersions]);
  ...
  const handleRestoreVersion = useCallback(async (versionId: string) => {
    ...
    try {
      ...
      // Reload restored content into editor
  ...
  const handleAccept = useCallback((id: string) => {
    apiFetch(`/suggestions/${id}`, { method: 'PATCH', body: JSON.stringify({ status: 'accepted' }) })
      .then(() => setSuggestions((prev) => prev.map((s) => s.id === id ? { ...s, status: 'accepted' as const } : s)))
      .catch(() => {});
  }, []);

  const handleReject = useCallback((id: string) => {
    apiFetch(`/suggestions/${id}`, { method: 'PATCH', body: JSON.stringify({ status: 'rejected' }) })
      .then(() => setSuggestions((prev) => prev.map((s) => s.id === id ? { ...s, status: 'rejected' as const } : s)))
      .catch(() => {});
  }, []);
  ...
  const handleFileUpload = useCallback(() => {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        // Trigger library panel re-fetch so the uploaded PDF appears immediately
        setSourcesRefreshKey((k) => k + 1);
